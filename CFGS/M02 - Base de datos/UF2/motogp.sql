...
SELECT p.nombre FROM pilotos p, resultados r WHERE p.codigo = r.codpiloto AND p.nombre NOT IN (SELECT p.nombre FROM pilotos p, resultados r WHERE p.codigo = r.codpiloto AND (r.puesto = 1 OR r.puesto = 2 OR r.puesto = 3));
#2. Mostra quin pilots, i en quina cursa, han quedat en última posició (tenint en compte els abandonaments).
SELECT p.nombre, r.nombregp, r.puesto FROM pilotos p, resultados r WHERE p.codigo = r.codpiloto AND r.puesto IN (SELECT r.puesto FROM resultados r ORDER BY r.puesto DESC) GROUP BY r.nombregp;
SELECT r.codpiloto, r.nombregp, r.puesto FROM resultados r WHERE r.puesto = (SELECT MIN(r.puesto) FROM resultados r) GROUP BY r.nombregp;
SELECT r.puesto, r.nombregp FROM resultados r GROUP BY r.nombregp ORDER BY r.puesto DESC;
#3. (1’25 punts) Mostra el dissenyador, en el cas de què es sàpiga, del circuit amb major longitud.
SELECT ci.disenyador FROM circuitos ci ORDER BY ci.longitud DESC LIMIT 1;
#4. (1’25 punts) Mostra a quines curses no hi ha hagut cap abandonament.
SELECT DISTINCT r.nombregp FROM resultados r WHERE r.nombregp NOT IN (SELECT DISTINCT r.nombregp FROM carreras c, resultados r WHERE r.distanciaganador = 'Abandono');
#5. (1’25 punts) Mostra amb quina/es quines escuderia/es s’han guanyat més curses.
SELECT p.nombreescuderia, COUNT(r.puesto) FROM pilotos p, resultados r WHERE p.codigo = r.codpiloto AND r.puesto = 1 GROUP BY p.nombreescuderia;
#6. (1’25 punts) Mostra el nom del pilot que hagi fet la major distància de temps respecte el guanyador.
SELECT p.nombre, r.distanciaganador FROM pilotos p, resultados r WHERE p.codigo = r.codpiloto AND r.distanciaganador != NULL OR r.distanciaganador != 'Abandono' ORDER BY r.distanciaganador DESC LIMIT 1;
#7. (1’25 punts) Mostra quina nacionalitat/s hi ha més pilots.
SELECT p.nacionalidad, COUNT(p.nombre) FROM pilotos p GROUP BY p.nacionalidad ORDER BY COUNT(*) DESC LIMIT 1;
SELECT p.nacionalidad, COUNT(p.nombre) FROM pilotos p GROUP BY p.nacionalidad ORDER BY COUNT(*) ASC LIMIT 1;
#8. (1’25 punts) Mostra en quina cursa van debutar més pilots i la quantitat d’aquests.
SELECT p.carreradebut, COUNT(p.nombre) FROM pilotos p GROUP BY p.carreradebut ORDER BY COUNT(*) DESC LIMIT 1;